...
interface CustomTooltipProps {
    ...
    payload?: Array<{
        color: string;
        name: string;
        value: number; // изменено с string на number
        payload: {
            name: string;
            value: number;
            color: string;
...
interface Conditions {
    lowHemoglobin: string;
    highHemoglobin: string;
    lowWbc: string;
    highWbc: string;
    lowRbc: string;
    highRbc: string;
    lowPlatelets: string;
    highPlatelets: string;
    lowGlucose: string;
    highGlucose: string;
...
const CustomTooltip = ({active, payload}: CustomTooltipProps) => {
    ...
    return null;
}, CustomLegend = ({payload}: { payload: LegendPayload[] }) => (
...
), CustomBarChart = ({data}: { data: ChartData[] }) => (
...
), {Title, Text} = Typography, BloodAnalysis: React.FC = () => {
    ...
    const getDiseaseRisks = (data: BloodTestFormValues): string[] => {
        const risks: string[] = [];
        const conditions = t('bloodAnalysis.warnings.conditions', {returnObjects: true}) as Conditions;
    ...
    const diseaseExplanations = t('bloodAnalysis.warnings.explanations') as unknown as DiseaseExplanations;
    const interestingFacts = t('bloodAnalysis.facts', { returnObjects: true }) as InterestingFacts;

    interface DiseaseExplanations {
        [key: string]: string;  // индексная сигнатура, позволяющая использовать строковые ключи
    }
    ...
    interface InterestingFacts {
        ...
        RBC: string[];
        WBC: string[];
        [key: string]: string[]; // добавляем индексную сигнатуру для безопасного доступа по строке
    }
            ...
            >
                <Col className="welcome-section-column">
                    ...
                    <Title
                        ...
                        style={{
                            fontSize:
                                width > 800 ? (language === 'hy' ? '35px' : '50px') : '30px',
                        }}
                    >
                    ...
                    >
                        ...
                        <ul style={{paddingLeft: 20}}>
                            {getDiseaseRisks(bloodTestData).length > 0 ? (
                                getDiseaseRisks(bloodTestData).map((risk: string, index) => (
                        ...
                        {chartData.map((item) => (
                            ...
                            >
                                ...
                                <div style={{marginTop: 15}}>
                                    {interestingFacts[item.name].map((fact: string, i: number) => (
